...
def run(root, mode, split, init_lr=0.1, max_steps=5000, batch_size=1, save_model=''):
    # setup dataset
    train_split= os.path.join(root,'splits/train')
    val_split= os.path.join(root,'splits/val')
    train_transforms = transforms.Compose([videotransforms.RandomCrop(224),videotransforms.RandomHorizontalFlip(),
    ])
    test_transforms = transforms.Compose([videotransforms.CenterCrop(224)])

    dataset = Dataset(train_split, split, root, mode, train_transforms)
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True, pin_memory=True)

    val_dataset = Dataset(val_split, split, root, mode, test_transforms)
...
if __name__ == '__main__':
    ...
    run(root=args.root, mode=args.mode, split=args.split, save_model=args.save_model)
